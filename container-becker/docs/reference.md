...
| `Limit`       | `limit` (risk), scaled ×1000 (API reports thousand EUR)                            |
...
Amounts are formatted German-style (decimal comma, `;` separator, UTF-8 with BOM for Excel). The credit limit arrives from the API in thousand EUR (e.g. `2` = 2,000 EUR) and is multiplied by 1000 on export; `Gekauft` and `balance` are already real EUR.
...
| `FreeLineFromBalance`                     | `false`                      | `true` = freie Linie = limit − balance, otherwise limit − purchased (limit scaled ×1000 in both cases)                                                                                                                                                                                                                                                                                                                              |
